...
#endif */
	//system("sudo cp -rf ./guppi_PSRFITS_v3.4_search_template.txt /usr/local/share");
	sprintf(template_file, "/usr/local/share/guppi_PSRFITS_v3.4_search_template.txt");
...
int psrfits_write_subint(struct psrfits *pf) {
    ...
    fits_write_col(pf->fptr, TFLOAT, 15, row, 1, nivals, sub->datI_offsets, status);
    fits_write_col(pf->fptr, TFLOAT, 16, row, 1, nivals, sub->datI_scales, status);
  //  if (mode==search) {
