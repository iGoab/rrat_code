 ...
 * This is designed to solve the Nancy survey problem of -ve spikes. 
 */
...
void inputClip(float *block, int nSamplesRead, float *clipBelow, float *clipAbove, int clipmode) {
	 ...
	 * Either of these are set if we should use the median as the threshold
	 * 00001100 = 12
	 */
	if (clipmode & 12) {
		...
		*/ 
		if (nbits > 16) {
			fprintf(stderr, "Error: Median computation not surpported for nbits > 16\n");
			exit(1);
		}
		 ...
		 * We can quickly do 2^n by left shifting by n.
		 */
		...
		/*
		 * Now to comput the histogram.
		 * Basicaly we take the closet integer ( to prevent errors from float rouding)
