...
Pulsus *pulses;
{
	...
	/*
	   Run through the time series again, this time comparing each sample with the threshold
	   Note: The important thing, when figuring out the index in the time series,
	   is not how many times the time series has been smoothed, but how many times it's been
	   *decimated*. The way single_ch() is currently structrued, the number of times the series
	   has been decimated is (ns - 1), except for ns == 0.
	   * 
	   Also, adjust the mean and rms to remove pulses. This prepares for the final pass through
	   the series below. 
	 */
	 *npulse = 0;
	 if (ns) 
